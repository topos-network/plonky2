...
/// the first three memory channels, and the result in next row's first channel
/// (binary operations do not use the third inputs).
///
...
pub fn ctl_data_partial_memory<F: Field>(is_push_write: bool) -> Vec<Column<F>> {
    let channel_map = COL_MAP.partial_channel;
    let values = if is_push_write {
        COL_MAP.mem_channels[0].value
    } else {
        COL_MAP.mem_channels[3].value
    };
    let mut cols: Vec<_> = Column::singles([
        channel_map.is_read,
        channel_map.addr_context,
...
pub fn ctl_filter_partial_memory<F: Field>(is_push_write: bool) -> Column<F> {
    if is_push_write {
        Column::single(COL_MAP.partial_channel.is_push_write)
    } else {
        Column::single(COL_MAP.partial_channel.is_mstore_general)
    }
}
