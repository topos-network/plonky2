...
pub(crate) fn eval_bootstrap_kernel_packed<F: Field, P: PackedField<Scalar = F>>(
    ...
    yield_constr.constraint(local_is_bootstrap * local_values.partial_channel.is_push_write);
    yield_constr.constraint(local_is_bootstrap * local_values.partial_channel.is_mstore_general);
    ...
    yield_constr.constraint(delta_is_bootstrap * local_values.partial_channel.is_push_write);
    yield_constr.constraint(delta_is_bootstrap * local_values.partial_channel.is_mstore_general);
...
pub(crate) fn eval_bootstrap_kernel_ext_circuit<F: RichField + Extendable<D>, const D: usize>(
    ...
    }
    {
        let constr = builder.mul_extension(
            local_is_bootstrap,
            local_values.partial_channel.is_push_write,
        );
    ...
    }
    {
        let constr = builder.mul_extension(
            local_is_bootstrap,
            local_values.partial_channel.is_mstore_general,
        );
        yield_constr.constraint(builder, constr);
    }
