...
impl<F: Field> GenerationState<F> {
    ...
    fn get_current_code(&mut self) -> Result<Vec<u8>, ProgramError> {
        self.get_code(self.registers.context)
    }

    fn get_code(&mut self, context: usize) -> Result<Vec<u8>, ProgramError> {
    ...
    fn get_code_len(&mut self, context: usize) -> Result<usize, ProgramError> {
